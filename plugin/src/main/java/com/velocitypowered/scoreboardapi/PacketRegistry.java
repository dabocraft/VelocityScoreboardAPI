...
public class PacketRegistry {
    ...
    public static void registerPackets(@NonNull ProtocolVersion maximumSupportedVersion) throws ReflectiveOperationException {
        StateRegistry.PacketRegistry clientbound = getClientboundRegistry();
    ...
    /**
     * Retrieves the clientbound PacketRegistry from StateRegistry.PLAY using reflection.
     * Tries known field names for robustness across Velocity snapshot builds.
     */
    @NonNull
    private static StateRegistry.PacketRegistry getClientboundRegistry() throws ReflectiveOperationException {
        // Try primary field name first
        String[] fieldNames = {"clientbound", "CLIENTBOUND", "client"};
        for (String name : fieldNames) {
            try {
                Field f = StateRegistry.class.getDeclaredField(name);
                f.setAccessible(true);
                Object value = f.get(StateRegistry.PLAY);
                if (value instanceof StateRegistry.PacketRegistry) {
                    return (StateRegistry.PacketRegistry) value;
                }
            } catch (NoSuchFieldException | IllegalAccessException ignored) {}
        }
        // Fallback: scan all fields of StateRegistry for a PacketRegistry typed field
        // clientbound is typically the second PacketRegistry field (serverbound is first)
        StateRegistry.PacketRegistry last = null;
        for (Field f : StateRegistry.class.getDeclaredFields()) {
            f.setAccessible(true);
            try {
                Object value = f.get(StateRegistry.PLAY);
                if (value instanceof StateRegistry.PacketRegistry) {
                    String fname = f.getName().toLowerCase();
                    if (fname.contains("client") || fname.contains("out") || fname.contains("send")) {
                        return (StateRegistry.PacketRegistry) value;
                    }
                    last = (StateRegistry.PacketRegistry) value; // keep last as fallback
                }
            } catch (IllegalAccessException ignored) {}
        }
        if (last != null) return last;
        throw new NoSuchFieldException(
            "VelocityScoreboardAPI: Could not find clientbound PacketRegistry field in StateRegistry. " +
            "Please report this at https://github.com/NEZNAMY/VelocityScoreboardAPI/issues"
        );
    }
    ...
    private static StateRegistry.PacketMapping map(int id, ProtocolVersion version) throws ReflectiveOperationException {
        // Try the standard 3-arg form (id, version, encodeOnly) used in most Velocity builds
        try {
            Method m = StateRegistry.class.getDeclaredMethod("map", int.class, ProtocolVersion.class, boolean.class);
            m.setAccessible(true);
            return (StateRegistry.PacketMapping) m.invoke(null, id, version, false);
        } catch (NoSuchMethodException ignored) {}
        // Fallback: try 2-arg form (id, version) in case signature changed
        try {
            Method m = StateRegistry.class.getDeclaredMethod("map", int.class, ProtocolVersion.class);
            m.setAccessible(true);
            return (StateRegistry.PacketMapping) m.invoke(null, id, version);
        } catch (NoSuchMethodException ignored) {}
        throw new NoSuchMethodException(
            "VelocityScoreboardAPI: Could not find StateRegistry.map(int, ProtocolVersion[, boolean]) " +
            "in this Velocity build. Please report at https://github.com/NEZNAMY/VelocityScoreboardAPI/issues"
        );
    }
    ...
    private static StateRegistry.PacketMapping map(int id, ProtocolVersion version, ProtocolVersion lastValidProtocolVersion) throws ReflectiveOperationException {
        // Try the standard 4-arg form (id, version, lastValid, encodeOnly)
        try {
            Method m = StateRegistry.class.getDeclaredMethod("map", int.class, ProtocolVersion.class, ProtocolVersion.class, boolean.class);
            m.setAccessible(true);
            return (StateRegistry.PacketMapping) m.invoke(null, id, version, lastValidProtocolVersion, false);
        } catch (NoSuchMethodException ignored) {}
        // Fallback: try 3-arg form without encodeOnly
        try {
            Method m = StateRegistry.class.getDeclaredMethod("map", int.class, ProtocolVersion.class, ProtocolVersion.class);
            m.setAccessible(true);
            return (StateRegistry.PacketMapping) m.invoke(null, id, version, lastValidProtocolVersion);
        } catch (NoSuchMethodException ignored) {}
        // Last resort: map without upper bound (may register for additional versions but won't crash)
        return map(id, version);
    }
}
